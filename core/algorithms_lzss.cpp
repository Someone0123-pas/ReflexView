...
void LZSS::decompress() {
    ...
    unsigned buffersize {static_cast<unsigned>(compressed[1] | compressed[2] << 0x8 | compressed[3] << 0x10)};
    ...
    while (decomp_tiles_index < buffersize) {
        ...
        if (flagbyte & (1 << flagnum)) {  // Compressed Byte
            unsigned len {static_cast<unsigned>(compressed[curindex] >> 0x4) + 3 + decomp_tiles_index};
...
void LZSS::dump_png(const std::string& filepath, unsigned width) {
    if (!decompressed) decompress();
    std::ofstream outputfile {filepath, std::ios::binary};
    auto [pngbuffer, pngsize] {png::from_4bpp(decompressed, size, width * 8)};
